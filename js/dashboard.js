...
$(document).ready(function () {
    ...
    $("#search-url").keyup(function () {
        ...
        if (value.length >= minlength) {
            ...
            //Get JSON data from report_json function + value passed from the URL paramater.   
            var url = '/search/report_url_json/' + value;
            ...
            $.getJSON(url, function (result) {
                ...
                if (result == undefined || result == null || result.length == 0) {
                    ...
                    $.each(result, function (i, report) {
                        // For each record in the returned array
                        $('#report-url').append('<li class="list-group-item"><span class="badge low-badge">' + report.request_number + '</span><strong><a href="/index/report/' + report.reportid + '">' + report.scanned_url + '</a></strong></li>');
                    });
